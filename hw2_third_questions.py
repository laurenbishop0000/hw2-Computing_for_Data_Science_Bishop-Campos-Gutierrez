...
def total_registered_cases_per_country(data):
    dictionary = {}
    for country in data:
        dictionary[country] = total_registered_cases(data, country)
    return dictionary
...
def country_with_most_cases(data):
    country_count = total_registered_cases_per_country(data)
    most_cases = ['',0]
    for x in country_count:
        if(country_count[x]>most_cases[1]):
            most_cases[1] = country_count[x]
            most_cases[0] = x
    return tuple(most_cases)